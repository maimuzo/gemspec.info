...
class SessionsController < ApplicationController
  ...
  #
  open_id_consumer :required => [:email, :nickname], :if_available => [:gender, :birth_date, :country, :language, :timezone]
  ...
  def begin
    ...
    case open_id_request.status
      when OpenID::Consumer::SUCCESS
        ...
        redirect_to open_id_request.redirect_url((request.protocol + request.host_with_port + "/"), complete_sessions_url)
      else
    ...
    case open_id_response.status
      ...
      when OpenID::Consumer::SUCCESS
        ...
        if open_id_fields.any?
          @user   = User.find_by_claimed_id(open_id_response.identity_url)
          @user ||= User.new(:claimed_id => open_id_response.identity_url)
          @user.email = open_id_fields['email']    if open_id_fields['email']
          @user.nickname = open_id_fields['nickname'] if open_id_fields['nickname']
          if @user.save
