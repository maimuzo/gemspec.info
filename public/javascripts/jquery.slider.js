...
jQuery.fn.makeSlider = function(scaleInterval, sliderWidth, selectsVisible){
	jQuery(this).each(function(){
		...
		if(jQuery(this).find('.sliderControl').size() > 0) return jQuery(this);
		//make a unique className for finding this slider
		function returnUnique(){
			var ranNum = Math.floor(Math.random()*100);
			if(jQuery('.sliderComponent_'+ranNum).size()==0) return ranNum;
			else returnUnique();
		}
		var uniqueID = returnUnique();
		
		jQuery(this).addClass('sliderComponent_'+uniqueID);
		var sliderComponent = jQuery('.sliderComponent_'+uniqueID);
		...
		if(firstSelect.find('optgroup').size()>0){
			...
			firstSelect.find('optgroup').each(function(i){
				var optName = jQuery(this).attr('label');
				scale[i] = {};
				scale[i].name = optName;
				scale[i].values = [];
				jQuery(this).find('option').each(function(j){
					scale[i].values[j] = {};
					scale[i].values[j].value = jQuery(this).attr('value');
					scale[i].values[j].label = jQuery(this).text();
				});
			});
		}
		//if no optgroups, make array of values,labels for each tic
			...
			firstSelect.find('option').each(function(j){
					scale[j] = {};
					scale[j].value = jQuery(this).attr('value');
					scale[j].label = jQuery(this).text();
				});
		}
		...
		sliderComponent.find('select').each(function(i){
			indicators.push(jQuery(this).attr('name'));
		});
		...
		if(optGroups) {//write dl
			...
			jQuery(scale).each(function(){
				...
				jQuery(this.values).each(function(){
		...
		if(scaleInterval){
			...
			sliderComponent.find('.xScale dd, .xScale li').css('text-indent', -999999).each(function(i){
				if(inc == 0) {jQuery(this).css('text-indent', 0);}
				inc++;
		...
		if(optGroups) {
			...
			jQuery(scale[0].values).each(function(){
					allValues[uniqueID].push(this.value);
			});
			jQuery(scale[1].values).each(function(){
					allValues[uniqueID].push(this.value);
			});
		}
		else {
		...
		jQuery(indicators).each(function(l){
		...
		sliderComponent.find('.scale span').each(function(n){
			jQuery(this).css('left', (sliderComponent.width()-36) / (scaleLength-1) * n);
		});
		
		sliderComponent.find('.xScale dd, .xScale li').each(function(m){
			jQuery(this).width((sliderComponent.width()) / scaleLength);
			jQuery(this).css('left', (sliderComponent.width()-36) / (scaleLength) * m);
		});
		...
		sliderComponent.find('select').each(function(i){
			selectedInd[i] = jQuery(this).find('option:selected').attr('value');
			if(selectedInd[i]){
	...
	jQuery('.sliderIndicator').each(function(k){
		jQuery(this).attr('tabindex', k+1);
	});
